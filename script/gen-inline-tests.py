...
def process_file(src_filename, dest_filename):
    ...
    with open(dest_filename, "w") as dest_file:
        ...
        cache = []
        parser_on = False
        for line_num in range(len(src_file_content)):
            line = src_file_content[line_num]
            if line.find("#tests") != -1:
                ...
                func_raw = line.strip()
                add_tests(dest_file, func_raw, cache, line_num)
                cache = []
            else:
                if parser_on:
                    cache.append(line)
...
def add_tests(dest_file, func_raw, cache, line_num):
    for test_index in range(len(cache)):
        test_raw = cache[test_index]
        try:
            output = lib.test_parser.parse(func_raw, test_raw, line_num, test_index)
        except Exception, e:
